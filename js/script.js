...
// Small helper to update text safely
...
function initUserPanel(){
  ...
  function maybePushAlert(){
    if(state.moisture < 30){ state.alerts.unshift({time:new Date(), text:'Umidade baixa — irrigar', level:'warn'}); }
    if(state.ph < 5.8 || state.ph > 7.2){ state.alerts.unshift({time:new Date(), text:'pH fora do ideal', level:'warn'}); }
    if(state.light < 1000){ state.alerts.unshift({time:new Date(), text:'Baixa luminosidade detectada', level:'info'}); }
  ...
  var chartMoisture = new Chart(ctxM,{type:'line',data:{labels: state.historyMoisture.map((_,i)=>i),datasets:[{label:'Umidade',data:state.historyMoisture,fill:true,tension:0.3}]},options:{responsive:true,plugins:{legend:{display:false}}}});
  var chartTemp = new Chart(ctxT,{type:'line',data:{labels: state.historyTemp.map((_,i)=>i),datasets:[{label:'Temperatura',data:state.historyTemp,fill:true,tension:0.3}]},options:{responsive:true,plugins:{legend:{display:false}}}});
